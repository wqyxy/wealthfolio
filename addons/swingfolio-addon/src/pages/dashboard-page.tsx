...
import { AnimatedToggleGroup, Button, Card, CardContent, CardHeader, CardTitle, GainAmount, GainPercent, Icons, Page, PageContent, PageHeader, Skeleton } from '@wealthfolio/ui';
...
import { mergePositions } from '../lib/merge-positions';
...
export default function DashboardPage({ ctx }: DashboardPageProps) {
  ...
  const [mergeMode, setMergeMode] = useState<'off' | 'symbol' | 'asset'>('symbol');
  ...
  const { baseCurrency, convertToBaseCurrency, exchangeRates } = useCurrencyConversion({ ctx });
  ...
  const exportToCSV = (openPositions: OpenPosition[]) => {
    ...
    const positionsToExport = mergeMode !== 'off' ? mergePositions(openPositions, mergeMode, exchangeRates, baseCurrency, convertToBaseCurrency) : openPositions;
    ...
    if (mergeMode !== 'off') {
      ...
      const rows = positionsToExport.map(pos => [
        ...
        pos.marketValueBaseCurrency !== undefined
          ? pos.marketValueBaseCurrency.toFixed(2)
          : baseCurrency && convertToBaseCurrency
          ? convertToBaseCurrency(pos.marketValue, pos.currency).toFixed(2)
          : 'N/A',
        ...
        <PageContent>
          <div className="flex h-[calc(100vh-200px)] items-center justify-center">
            <div className="px-4 text-center">
              ...
              <h3 className="mb-2 text-base font-semibold sm:text-lg"> No Swing Trading Activities Selected </h3>
              <p className="text-muted-foreground mb-4 text-sm sm:text-base">
                {' '}
              ...
              <Button onClick={handleNavigateToActivities} className="mx-auto">
                <Icons.Plus className="mr-2 h-4 w-4" /> Select Activities
              </Button>
            </div>
          </div>
        </PageContent>
  ...
  const headerActions = (
    <>
      ...
      <Button variant="outline" className="hidden rounded-full sm:inline-flex" onClick={handleNavigateToActivities}>
        <Icons.ListChecks className="mr-2 h-4 w-4" /> <span>Select Activities</span>
      </Button>
        ...
        <div className="space-y-4 sm:space-y-6">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground text-xs"> Realized ({metrics.totalTrades} trades) </span>
                    <div className="flex items-center gap-2">
                      <GainAmount value={metrics.totalRealizedPL} currency={metrics.currency} className="font-medium" displayDecimal={false} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-muted-foreground text-xs"> Unrealized ({metrics.openPositions} open) </span>
                    <div className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-semibold"> {metrics.averageHoldingDays.toFixed(1)} days </span>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="flex flex-col">
              <CardHeader className="shrink-0 pb-3">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <CardTitle className="text-base sm:text-lg"> {getChartPeriodDisplay(selectedPeriod).type} Equity Curve </CardTitle>
                    <p className="text-muted-foreground text-xs sm:text-sm"> {getChartPeriodDisplay(selectedPeriod).description} </p>
                  </div>
          ...
          <Card>
            {/* <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <span className="text-muted-foreground text-sm"> {openPositions.length} {openPositions.length === 1 ? 'position' : 'positions'} </span>
            </CardHeader> */}
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              >
                <Icons.Download className="mr-2 h-4 w-4" /> Export to CSV
              </Button>
            </CardHeader>
            <CardContent className="px-2 sm:px-6">
              <OpenTradesTable
                positions={mergeMode !== 'off' ? mergePositions(openPositions, mergeMode, exchangeRates, baseCurrency, convertToBaseCurrency) : openPositions}
